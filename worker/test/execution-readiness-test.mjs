...

{
  ...
  assert.equal(readiness.external_signer.selected, true)
  assert.equal(readiness.external_signer.submission_runner_configured, false)
  assert.equal(readiness.external_signer.unavailable_code, 'WAAP_RUNNER_MISSING')
  assert.equal(readiness.signer_capabilities.some((row) => row.kind === SIGNER_KIND_WAAP && row.runner_configured === false), true)
  assert.equal(readiness.funding_ready, true)
  assert.equal(readiness.execution_ready, false)
...

{
  ...
  assert.equal(readiness.external_signer.status, 'available')
  assert.equal(readiness.external_signer.submission_runner_configured, true)
  assert.equal(readiness.signer_capabilities.some((row) => row.kind === SIGNER_KIND_WAAP && row.execution_enabled === true), true)
  assert.equal(readiness.funding_ready, true)
  assert.equal(readiness.execution_ready, true)
  assert.equal(readiness.execution_claimed, false)
}

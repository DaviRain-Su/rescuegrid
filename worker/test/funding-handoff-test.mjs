...
assert.equal(handoff.funding_targets.balance_manager.required_assets[0].target_kind, 'deepbook_balance_manager')
assert.equal(handoff.funding_targets.balance_manager.required_assets[0].target, DEPLOYMENT.agent.balance_manager_id)
assert.equal(handoff.funding_targets.balance_manager.required_assets[0].destination_object_id, DEPLOYMENT.agent.balance_manager_id)
assert.equal(handoff.funding_targets.balance_manager.required_assets[0].destination_address, null)
assert.equal(handoff.funding_targets.balance_manager.required_assets[0].funding_action, 'deepbook_balance_manager_deposit')
assert.equal(handoff.funding_targets.balance_manager.required_assets[0].direct_wallet_transfer_accepted, false)
assert.match(handoff.funding_targets.balance_manager.required_assets[0].proof_requirement, /BalanceManager/)
assert.equal(handoff.funding_targets.balance_manager.required_assets[1].missing, '10')
assert.equal(handoff.funding_targets.balance_manager.target_kind, 'deepbook_balance_manager')
assert.equal(handoff.funding_targets.balance_manager.direct_wallet_transfer_accepted, false)
assert.equal(handoff.funding_targets.agent_gas.required_assets[0].missing, '0')
assert.equal(handoff.funding_targets.agent_gas.required_assets[0].target_kind, 'agent_gas_address')
assert.equal(handoff.funding_targets.agent_gas.required_assets[0].target, DEPLOYMENT.agent.address)
assert.equal(handoff.funding_targets.agent_gas.required_assets[0].destination_object_id, null)
assert.equal(handoff.funding_targets.agent_gas.required_assets[0].destination_address, DEPLOYMENT.agent.address)
assert.equal(handoff.funding_targets.agent_gas.required_assets[0].funding_action, 'sui_gas_transfer')
assert.equal(handoff.funding_targets.agent_gas.required_assets[0].direct_wallet_transfer_accepted, true)
assert.equal(handoff.funding_targets.agent_gas.target_kind, 'agent_gas_address')
assert.equal(handoff.funding_targets.agent_gas.direct_wallet_transfer_accepted, true)
assert.deepEqual(handoff.funding_routing.balance_manager_assets.required_assets, ['DBUSDC', 'DEEP'])
assert.equal(handoff.funding_routing.balance_manager_assets.target_kind, 'deepbook_balance_manager')
assert.equal(handoff.funding_routing.balance_manager_assets.target, DEPLOYMENT.agent.balance_manager_id)
assert.equal(handoff.funding_routing.balance_manager_assets.accepted_action, 'deepbook_balance_manager_deposit')
assert.equal(handoff.funding_routing.balance_manager_assets.rejected_action, 'direct_dbusdc_or_deep_transfer_to_agent_wallet')
assert.equal(handoff.funding_routing.balance_manager_assets.direct_wallet_transfer_accepted, false)
assert.match(handoff.funding_routing.balance_manager_assets.proof_requirement, /DBUSDC\/DEEP target asset hits/)
assert.deepEqual(handoff.funding_routing.agent_gas.required_assets, ['SUI_MIST'])
assert.equal(handoff.funding_routing.agent_gas.target_kind, 'agent_gas_address')
assert.equal(handoff.funding_routing.agent_gas.target, DEPLOYMENT.agent.address)
assert.equal(handoff.funding_routing.agent_gas.accepted_action, 'sui_gas_transfer')
assert.equal(handoff.funding_routing.agent_gas.direct_wallet_transfer_accepted, true)
assert.equal(handoff.funding_routing.strict_execution_report_required, true)
...
assert.equal(handoff.next_verification.funding_proof_command, 'npm run funding:proof -- --tx <provider_funding_tx_digest> --json')
assert.equal(handoff.next_verification.funding_proof_report_command, 'npm run funding:proof:report -- --tx <provider_funding_tx_digest>')
...
assert.match(markdown, /Funding routing:/)
assert.match(markdown, /DBUSDC\/DEEP destination: DeepBook BalanceManager/)
assert.match(markdown, /Do not transfer DBUSDC or DEEP directly to the agent wallet/)
assert.match(markdown, /proof requires BalanceManager target asset hits and live BalanceManager reads/)
assert.match(markdown, /SUI gas destination: agent address/)
assert.match(markdown, /After funding, run:/)
assert.match(markdown, /npm run funding:watch:report/)
assert.match(markdown, /npm run funding:proof -- --tx <provider_funding_tx_digest> --json/)
assert.match(markdown, /npm run funding:proof:report -- --tx <provider_funding_tx_digest>/)
...
try {
  ...
  assert.match(artifactBody, /target_kind deepbook_balance_manager/)
  assert.match(artifactBody, /Do not transfer DBUSDC or DEEP directly to the agent wallet/)
  assert.match(artifactBody, /readiness-only/)
  assert.match(artifactBody, /funding:proof/)

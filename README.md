...
npm run demo:execute:wallet-report -- --wrapper-id <id> --strategy-file <json> --create-tx-digest <digest>
...
> Wallet click-through evidence path: `npm run wallet:evidence -- --format markdown --out .rescuegrid/wallet-clickthrough-evidence.md` creates a gitignored manual evidence artifact for the real Slush / standard Sui wallet flow. The script is read-only: it snapshots frontend reachability/login guardrails plus public Worker root/runtime/readiness/chain-data status, but it does not create a policy, submit a PTB or claim a live wallet click-through. Run `npm run wallet:evidence:preflight` before manual QA to require the local frontend, login boundary and Worker public status to be ready. During the browser run, set `Actual click-through completed: true` and fill the artifact with owner address, create tx digest, `wrapper_id`, `mandate_id`, strategy hash, `activation_strategy_file`, revoke tx digest, `strict_execution_report_reference`, and screenshots/evidence references for sign-in, create approval, active policy, activity rows, revoke approval and revoked state. After a wallet create succeeds, the UI shows an Activation strategy evidence banner; download that JSON and record its local path as `activation_strategy_file`, or run `npm run wallet:evidence:apply-strategy -- --input .rescuegrid/wallet-clickthrough-evidence.md --strategy-file <activation_strategy_file>` to fill only the machine-derived owner/create/wrapper/mandate/hash fields while leaving screenshots, revoke digest and click-through completion manual. Keep the same wallet-created wrapper active for the strict execution evidence window; after DBUSDC/DEEP funding is ready, run `npm run demo:execute:wallet-report -- --wrapper-id <wrapper_id> --strategy-file <activation_strategy_file> --create-tx-digest <create_tx_digest>`, revoke from the browser wallet when the script prints `awaiting_wallet_revoke`, and let it write `.rescuegrid/demo-execute-report.json`. Then run `npm run wallet:evidence:apply-report -- --input .rescuegrid/wallet-clickthrough-evidence.md --execution-report .rescuegrid/demo-execute-report.json` to fill only the machine-derived strict report reference, revoke digest and post-revoke status while keeping screenshots and click-through completion manual. Final `mission:readiness` requires the wallet artifact and strict report to describe the same wrapper lifecycle. `strict_execution_report_reference` must point to the strict report path that `mission:readiness -- --execution-report ...` will read. After filling it, run `npm run wallet:evidence:verify -- --input .rescuegrid/wallet-clickthrough-evidence.md --require-worker --execution-report .rescuegrid/demo-execute-report.json` to reject obvious secret assignments, verify local screenshot evidence files are readable when local paths are used, verify activation reached `Monitoring` and post-revoke status is `revoked`, verify the activation strategy JSON hashes to `strategy_hash`, verify the strict execution report is a wallet-created structured `AgentTradeExecuted` proof for the same owner/wrapper/mandate/hash/create/revoke lifecycle, then check the create/revoke digests against Sui `PolicyCreated` / `PolicyRevoked` events and required Worker detail reads.
...
> Strict execution validation path: after the BalanceManager is funded with usable DBUSDC/DEEP and `EXECUTION_ENABLED=true`, run `npm run demo:execute` or `npm run demo:execute:report` for the self-contained scripted-owner strict loop. It first preflights `/api/runtime/status`, BalanceManager DBUSDC/DEEP, and agent SUI gas before creating any policy; if the preflight is blocked, the script fails without leaving a test policy behind. Once preflight passes, it uses the same live loop but fails unless the forced tick produces structured `AgentTradeExecuted` evidence, `execution_claimed=true` and an on-chain spend increase. For final browser-wallet same-wrapper readiness, use `npm run demo:execute:wallet-report -- --wrapper-id <wallet_wrapper_id> --strategy-file <activation_strategy_file> --create-tx-digest <wallet_create_tx_digest>` while the wallet-created policy is still active; that script does not create or revoke the policy, waits for the owner to revoke in the browser wallet, then writes `.rescuegrid/demo-execute-report.json` only after create -> execute -> wallet revoke -> post-revoke no-execution passes. Both strict report paths require distinct create/tick/revoke tx digests, create/revoke success tx timestamps, all G2 loop assertions, public `AgentTradeExecuted` fields for the same wrapper/mandate/tick digest, create <= execute <= revoke timestamp order, and a post-revoke `POLICY_REVOKED` / `execution_claimed=false` no-execution proof for the same wrapper.
...
> Funding/execution gate: the deployed agent BalanceManager is currently unfunded for execution (`DBUSDC=0`, `DEEP=0` in final validation). Readiness surfaces correctly remain blocked with labels such as `EXECUTION_DISABLED`, `INSUFFICIENT_DBUSDC`, and `INSUFFICIENT_DEEP`. `npm run funding:request` produces a secret-safe handoff with public agent / BalanceManager ids, DBUSDC/DEEP coin types, observed balances, missing amounts and the exact verification commands to rerun after external funding; signer and external-signer posture is allowlisted before serialization so raw tokens, sessions, runner output and key material cannot enter the artifact. Add `--format markdown --out .rescuegrid/funding-request.md` to create a gitignored artifact for a funding provider. The handoff and funding-watch JSON include `execution_gate.readiness_only=true` and `strict_execution_report_required=true`: even a ready funding gate is not execution evidence until a strict report writes `.rescuegrid/demo-execute-report.json`; final browser-wallet readiness should use `npm run demo:execute:wallet-report` for the wallet-created wrapper. `npm run funding:watch -- --json` is the repeatable no-policy readiness check, `npm run funding:watch:report` persists it as `.rescuegrid/funding-watch-report.json`, and `npm run funding:watch -- --wait --run-demo --worker-url http://localhost:8787` can hand off to the self-contained strict execution validator only after readiness is true. Real DeepBook execution was explicitly deferred/skipped until usable Testnet DBUSDC/DEEP funding exists; this repo must not claim a successful live DeepBook fill yet.
...
- `npm run wallet:evidence -- --format markdown --out .rescuegrid/wallet-clickthrough-evidence.md` is the read-only browser-wallet evidence artifact generator. `npm run wallet:evidence:preflight` checks local frontend reachability, source-level login guardrails and Worker public readiness before manual wallet QA. The filled artifact must keep the wallet-created wrapper active for strict execution evidence before the wallet revoke step, record the UI-downloaded activation strategy JSON path as `activation_strategy_file`, and fill `strict_execution_report_reference` for that same lifecycle. `npm run wallet:evidence:apply-strategy -- --input .rescuegrid/wallet-clickthrough-evidence.md --strategy-file <activation_strategy_file>` reads the UI JSON, recomputes its canonical strategy hash, refuses secret-bearing or conflicting files, and fills only the owner/create/wrapper/mandate/hash evidence fields; it does not set `Actual click-through completed: true` or fill revoke/screenshot evidence. `npm run demo:execute:wallet-report -- --wrapper-id <wrapper_id> --strategy-file <activation_strategy_file> --create-tx-digest <create_tx_digest>` is the strict same-wrapper validator: it activates/forces execution against the wallet-created wrapper, waits for browser-wallet revoke at `awaiting_wallet_revoke`, then writes `.rescuegrid/demo-execute-report.json`. `npm run wallet:evidence:apply-report -- --input .rescuegrid/wallet-clickthrough-evidence.md --execution-report .rescuegrid/demo-execute-report.json` reads that wallet-created strict report, requires structured `AgentTradeExecuted` evidence, refuses secret-bearing or conflicting files, and fills only the strict report reference plus revoke/status lifecycle fields. `npm run wallet:evidence:verify -- --input .rescuegrid/wallet-clickthrough-evidence.md --require-worker --execution-report .rescuegrid/demo-execute-report.json` verifies a filled artifact by rejecting obvious secret assignments, requiring explicit click-through completion plus sign-in/create/revoke/active/revoked screenshot or evidence references, reading local screenshot files when local paths are provided, requiring activation state `Monitoring` and post-revoke status `revoked`, reading `activation_strategy_file` to verify its canonical hash plus owner/wrapper/mandate/create digest metadata, reading the strict report to verify wallet-created structured execution evidence plus owner/wrapper/mandate/hash/create/revoke continuity, requiring the strict report reference to match the expected report path, then checking the create/revoke tx digests against Sui `PolicyCreated` / `PolicyRevoked` events and required Worker detail reads. The generated blank artifact, apply-strategy/apply-report merges and preflight pass are not themselves proof that the browser wallet flow was clicked.
- `npm run mission:readiness` is the secret-safe final PRD gate. It checks required validation scripts, requires `.rescuegrid/safety-negative-report.json` from `npm run safety:negative:report`, verifies the filled wallet artifact, reads the same execution funding readiness contract used by `funding:watch`, allowlists signer capability / external-signer posture before serializing funding evidence, and keeps `execution_gate.readiness_only=true` / `strict_execution_report_required=true` in the aggregate funding check. It then requires `.rescuegrid/demo-execute-report.json`, preferably from `npm run demo:execute:wallet-report` for final browser-wallet readiness, proving the full create -> execute -> wallet revoke -> post-revoke strict loop: Testnet purpose/chain metadata, owner/wrapper/mandate/strategy ids, delegated agent, pool id, successful create and revoke txs with timestamps, distinct create/tick/revoke digests, all G2 assertions, structured `AgentTradeExecuted` evidence for the same wrapper/mandate/tick digest, matching delegated agent and pool, `execution_claimed=true`, spend increase, create <= execute <= revoke timestamp order, and post-revoke `POLICY_REVOKED` no-execution evidence. The aggregate gate also requires the verified browser wallet artifact and strict execution report to describe the same owner-created policy lifecycle: owner, wrapper, mandate, strategy hash and create/revoke tx digests must match. When the wallet artifact is incomplete, the blocked report includes click-through status, Worker URL and exact missing field counts, and points operators back through `wallet:evidence` generation, `wallet:evidence:preflight`, optional `wallet:evidence:apply-strategy` and `wallet:evidence:apply-report` before final manual QA. `npm run mission:readiness:report` writes the same aggregate JSON to `.rescuegrid/mission-readiness-report.json` even while blocked. In the current local validation with safety evidence present, it should still return non-zero with `status=blocked` because the browser wallet artifact is incomplete, DBUSDC/DEEP funding is not ready, and no strict execution report exists; on a fresh checkout, rerun `npm run safety:negative:report` first if the gitignored safety report is missing.

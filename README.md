...
- `npm run wallet:evidence -- --format markdown --out .rescuegrid/wallet-clickthrough-evidence.md` is the read-only browser-wallet evidence artifact generator. It records public Worker posture and the exact manual fields required for a real Slush / standard wallet create/revoke click-through. The generated artifact is not itself proof that the browser wallet flow was clicked.

...
Worker read surfaces also expose Sui-only composability metadata: `/api/protocols` returns the DefiLlama Sui top-26 plus volume exceptions registry, `/api/protocols/watchlist` maps that universe into protocol/venue/market/risk rows, and `/api/adapters/candidates` lists the H4/H5 DEX/lending target schemas and preflight gates. DeepBook appears as the only configured executor path, but watchlist rows keep `execution_enabled=false` with `FUNDING_GATED` until DBUSDC/DEEP funding is real; adapter candidates also stay `registered_executor=false`.

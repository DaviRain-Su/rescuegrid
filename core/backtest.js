...
export function runBacktest(prices, { thresholdPct = 8, budget = 500, slicePct = 20 } = {}) {
  ...
  let held = 0, trades = 0, spent = 0, peak = prices[0]
  const curve = []
  ...
  let mdPeak = curve[0], maxDd = 0
  for (const v of curve) { if (v > mdPeak) mdPeak = v; maxDd = Math.max(maxDd, (mdPeak - v) / mdPeak) }
  const stats = [

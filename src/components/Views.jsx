...
import { evidenceRowsFor, makeLedgerRow, outcomeOfActivity, policyLookup, signerEvidenceRowsFor } from '../activity-ledger.js'
...
export function ActivityView({ activity, policies = [], onTx, live = false, loading = false, source = null }) {
  ...
  const expand = (a) => {
    const oc = outcomeOfActivity(a)
    const px = RG.prices[(String(a.policy || '').match(/SUI|DEEP|WAL/) || ['SUI'])[0]] || RG.prices.SUI
    const reason = {

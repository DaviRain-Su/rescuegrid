              ...
              {feed.slice(0, 6).map((a, i) => (
                <div key={a.t + i} className={i === 0 && crashState === 'rescuing' ? 'fade-up' : ''}
                  ...
                  <div style={{ flexShrink: 0, width: 26, height: 26, borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center',
                    ...
                    color: a.kind === 'exec' ? 'var(--accent)' : a.kind === 'guardian' ? 'var(--danger)' : a.kind === 'rebalance' ? 'var(--sui)' : a.kind === 'policy' ? 'var(--sui)' : 'var(--t1)' }}>
                    <Icon name={a.kind === 'exec' ? 'bolt' : a.kind === 'guardian' ? 'shield' : a.kind === 'rebalance' ? 'swap' : a.kind === 'policy' ? 'grid' : 'eye'} size={14} />
                  </div>

...
Sui 的对象模型、PTB、zkLogin 和 Deepbook 让 RescueGrid 可以把“安全授权”和“自主执行”放在同一个可演示闭环里。后续协议覆盖以 DefiLlama Sui non-CEX Top 25/26 为监控池，但执行权限只给有交易量、清晰仓位操作面和可审计 PTB 约束的高流动性协议。优先执行候选包括 Cetus、Bluefin Spot、Turbos、Momentum、NAVI、Suilend、Scallop 和 AlphaLend；Bucket、Current、SpringSui、Haedal、Volo、AlphaFi、Kai、Mole、Ondo、KAIO、MatrixDock、Ember、Bluefin Pro、Sudo 和 DipCoin 先作为 watch-only 或 display-only 集成。

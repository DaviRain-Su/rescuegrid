...
Sui 的对象模型、PTB、zkLogin 和 Deepbook 让 RescueGrid 可以把“安全授权”和“自主执行”放在同一个可演示闭环里。后续协议覆盖以 DefiLlama Sui non-CEX Top 25/26 为监控池，但执行权限只给有交易量、清晰仓位操作面和可审计 PTB 约束的高流动性协议。优先执行候选包括 Cetus、Bluefin Spot、Turbos、Momentum、NAVI、Suilend、Scallop 和 AlphaLend；Bucket、Current、SpringSui、Haedal、Volo、AlphaFi、Kai、Mole、Ondo、KAIO、MatrixDock、Ember、Bluefin Pro、Sudo 和 DipCoin 先作为 watch-only 或 display-only 集成。Liquid Sui DEX 首先进入 quote/depth/spread read adapter 层，用来支持行情监控和机会扫描；Sui lending 首先进入 reserve/obligation/health-factor read adapter 层，用来支持 Lending Optimizer 和 borrow health guardian；这些 read layers 不等同于授予 swap、LP、repay、withdraw 或套利执行权限。
...
- Executor Adapter 层
  ...
  - 非 DeepBook 的 DEX read adapters 可以先暴露 quote/depth/spread 字段和 spread matrix，但在注册 ExecutorAdapter、wrapper 字段和 Guardian preflight 前不得构建或提交 PTB。
  - Lending read adapters 可以先暴露 reserve、obligation、health factor、liquidation buffer 和 repay dry-run 所需字段，但在 owner cap / obligation key / reserve id 约束进入 Wrapper 前不得执行 repay、withdraw 或 borrow。
...
- 可扩展性：策略模板应支持后续加入 DCA、网格、止损、对冲；协议执行通过 adapter registry 扩展，协议行情通过 read adapter registry 扩展，不能把 Deepbook 逻辑散落到 Policy、Guardian 或 Runtime 核心里。

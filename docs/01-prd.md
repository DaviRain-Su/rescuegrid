...
Sui 的对象模型、PTB、zkLogin 和 Deepbook 让 RescueGrid 可以把“安全授权”和“自主执行”放在同一个可演示闭环里。后续接入 Sui lending、CDP、CLMM、LST、vault、RWA 和 perps watch 协议时，应复用同一套 Policy、Guardian、Runtime 和 Activity 边界，只替换或新增 ExecutorAdapter；优先候选包括 Cetus、Turbos、Momentum、Scallop、NAVI、Suilend、AlphaLend、Bucket、SpringSui、Haedal、Volo、AlphaFi、Ondo、KAIO、Ember、Bluefin、Sudo 和 DipCoin。

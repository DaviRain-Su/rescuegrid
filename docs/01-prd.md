...
RescueGrid 是一个 Cloud-first、Local-extensible 的自主 DeFi 风险响应 Agent。它让用户通过 MoveGate Mandate + RescuePolicyWrapper 授权，把有限预算、允许交易范围、滑点、过期时间和撤销能力写入链上约束，然后允许 Agent 在这些约束内自主监控、风控、构建 PTB 并在 Deepbook 上执行风险响应交易。
...
差异化组合：Sui 原生 × Deepbook 集成 × 自然语言意图 × 风险响应场景 × zkLogin。

...
- **D** dashboard wiring — ✅ sign-in (Sui wallet primary, zkLogin optional); live create-policy (parse → build → wallet-sign → activate with strategy trigger); live D4 list/activity includes runtime stale markers + D5 revoke with `ALREADY_REVOKED` preflight; responsive + lazy-loaded landing; D3 live PTB preview; Active Strategy detail matches live runtime activity by wrapper id as well as display name. **Live write loop proven on-chain** (see below).

...
| **Worker API** (Cloudflare + Hono) | ✅ all endpoints | `npm test` + `npm run typecheck`; parse static Guardian block + runtime activity log + ExecutorAdapter registry + adapter target gate + active policy cap + revoke preflight covered |
...
- **E** Worker — ✅ E1–E9. parse (`executor_kind=deepbook`, unsupported strategy/executor errors, static slippage hard-cap `GUARDIAN_STATIC_BLOCK`, strategy_hash matches spec vectors) · create_policy PTB (zkLogin-signed, dry-run success) · active policy cap (`ACTIVE_POLICY_LIMIT_REACHED`) · activity (chain-authoritative + Durable Object runtime feed) · Guardian · tick state machine · Durable Object alarm · state sync · adapter registry with `UNSUPPORTED_EXECUTOR` and adapter target gating (`UNSUPPORTED_EXECUTOR_TARGET`).

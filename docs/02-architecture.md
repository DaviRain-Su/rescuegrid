...
当前实现入口是 `worker/src/runtime-core.js`：它承载 pure tick state machine、Guardian 调用、proposed trade 构造、ExecutorAdapter registry 选择、ExecutionPlan 准备和执行结果分类。ExecutorAdapter SDK 在 `worker/src/executor-adapter-sdk.js`，DeepBook 插件在 `worker/src/deepbook-adapter.js`，注册表组装在 `worker/src/executor-adapters.js`。`worker/src/tick.js` 负责链上读取、funding gate、SignerAdapter 和提交；`worker/src/risk-controls.js` 定义 owner-signed venue stop 控制消息和 runtime gate 匹配；`worker/scripts/daemon.mjs` 通过同一 tick/runtime-core 边界运行本地监控。

Venue emergency stop 是 runtime pre-submit gate：Dashboard 用 owner wallet 签名写入 Worker Durable Object，tick 在 trigger 命中后、PTB 签名前读取控制状态，命中 venue 时返回 `VENUE_STOPPED` 并写 runtime activity。它不会修改 MoveGate Mandate 或 `RescuePolicyWrapper`，所以不是链上权限撤销；链上撤销仍走 owner-signed revoke。

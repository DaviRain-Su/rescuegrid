...
- `PolicyReader`：读取链上 Policy object、事件和剩余额度。
- `MarketReader`：读取 Deepbook pool 状态、价格和预计滑点。
- `Guardian`：基于 Policy、market snapshot 和 proposed trade 返回 allow/block。
- `Executor`：构建 PTB 并以被授权 agent address 提交交易。
- `ActivityWriter`：写 runtime log，并在需要公开证明时调用 `record_guardian_block`。

MVP 只实现 Cloud Agent。Local Agent 后续复用这些接口，不改变 Move Policy surface。

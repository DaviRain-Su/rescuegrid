...
当前实现入口是 `worker/src/runtime-core.js`：它承载 pure tick state machine、Guardian 调用、proposed trade 构造、ExecutorAdapter registry 选择、ExecutionPlan 准备和执行结果分类。`worker/src/tick.js` 负责链上读取、funding gate、SignerAdapter 和提交；`worker/scripts/daemon.mjs` 通过同一 tick/runtime-core 边界运行本地监控。

...
当前实现入口是 `worker/src/runtime-core.js`：它承载 pure tick state machine、Guardian 调用、proposed trade 构造、ExecutorAdapter registry 选择、ExecutionPlan 准备和执行结果分类。ExecutorAdapter SDK 在 `worker/src/executor-adapter-sdk.js`，DeepBook 插件在 `worker/src/deepbook-adapter.js`，注册表组装在 `worker/src/executor-adapters.js`。`worker/src/tick.js` 负责链上读取、funding gate、SignerAdapter 和提交；`worker/src/risk-controls.js` 定义 owner-signed global / strategy / venue runtime control 消息和 gate 匹配；`worker/scripts/daemon.mjs` 通过同一 tick/runtime-core 边界运行本地监控。

Runtime risk controls 是 pre-submit gate：Dashboard 用 owner wallet 签名写入 Worker Durable Object，tick 在 trigger 命中后、PTB 签名前读取控制状态。命中 owner 全局 stop 返回 `GLOBAL_STOPPED`，命中 wrapper stop 返回 `STRATEGY_STOPPED`，命中 venue stop 返回 `VENUE_STOPPED`，并写 runtime activity。它不会修改 MoveGate Mandate 或 `RescuePolicyWrapper`，所以不是链上权限撤销；链上撤销仍走 owner-signed revoke。
...
- 提供 H7 SDK conformance：required interface、liquidity/volume gate、duplicate kind rejection、stable unsupported-executor errors。
...
- Lending read adapters 是独立只读层：`worker/src/sui-lending-read-adapters.js` 暴露 NAVI/Suilend/Scallop/AlphaLend 的 reserve、obligation、health-factor 和 liquidation-buffer 字段，但不注册 repay/withdraw/borrow 执行。
...
- 提供 `status`、`policies list`、`tick`、`logs`、`stop` 等操作入口；`status --json` 复用 `worker/src/execution-readiness.js`，与 Worker `/api/execution/readiness` 保持同一套 signer/funding blocker 语义。

...
| Agent runtime and Guardian | `agent-dev`, `wallets`, `ptbs` | Durable Object ticks, Runtime Core, signer adapters, risk controls, non-mutating validate-plan safety blockers | `npm --prefix worker test`, `npm run demo:loop`, `npm run demo:execute`, `npm run safety:negative`, `npm run funding:request` |

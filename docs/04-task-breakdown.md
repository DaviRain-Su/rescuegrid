...
Current K7 implementation status: Risk Center now exposes the full Sui-first risk-control surface: global risk and daily-loss budgets, strategy-level pause/resume controls, Sui venue stop/resume controls, venue exposure caps, liquidation watch, oracle health, signer/executor health, stale-data warning summaries, Guardian rule editor/simulator and the capability matrix. Global stop maps to the app-wide halt path, and strategy stop maps to the existing policy toggle. Venue stop now uses an owner-signed Sui personal message to persist a Worker Durable Object runtime control; tick reads that control and returns `VENUE_STOPPED` before PTB signing when a stopped venue would execute. This is a runtime pre-submit gate, not a MoveGate Mandate revoke.

...
| G7 | Commit | 1h | Browser wallet evidence artifact | `npm run wallet:evidence -- --format markdown --out .rescuegrid/wallet-clickthrough-evidence.md` 生成只读、gitignored 的 Slush/std wallet create/revoke 证据模板，列出 create tx、wrapper、mandate、revoke tx 和截图字段；`npm run wallet:evidence:verify -- --input .rescuegrid/wallet-clickthrough-evidence.md` 校验填好的 tx/event 一致性；生成模板不得被当作已完成 click-through |
| G8 | Commit | 1h | Mission readiness gate | `npm run mission:readiness` 汇总 validation scripts、safety-negative report、真实钱包证据、funding readiness 和 strict `AgentTradeExecuted` report；只有全部通过才返回 `status=ready`，当前资金/钱包/安全报告证据未完成时必须返回 `status=blocked` 且不得创建 policy 或提交 PTB |

Current G5 implementation status: `worker/scripts/funding-handoff.mjs`, `worker/scripts/funding-watch.mjs`, root `npm run funding:request` and `npm run funding:watch` reuse `buildExecutionReadiness` to produce a read-only external funding request/watch. The output includes public agent / BalanceManager ids, DeepBook pool, DBUSDC/DEEP coin types, observed/required/missing amounts, signer readiness, blocker codes and the next `npm run daemon -- status --json` / `npm run funding:watch -- --json` / `npm run demo:execute` verification commands. `npm run funding:request -- --format markdown --out .rescuegrid/funding-request.md` writes the same secret-safe handoff as a gitignored provider artifact. It does not create policies, submit PTBs or print secrets while blocked.
...
Current G7 implementation status: `scripts/wallet-clickthrough-evidence.mjs` is exposed at root as `npm run wallet:evidence` and `npm run wallet:evidence:verify`. It is read-only: it can fetch public Worker root/runtime/readiness/chain-data posture, writes a gitignored `.rescuegrid/wallet-clickthrough-evidence.md` artifact, keeps `actual_clickthrough_completed=false` until manually filled, records required owner/create/revoke/wrapper/mandate/screenshot evidence fields, and verifies a filled artifact against Sui `PolicyCreated` / `PolicyRevoked` transaction events plus optional Worker detail reads without claiming DeepBook execution.

Current G8 implementation status: `scripts/mission-readiness.mjs` is exposed at root as `npm run mission:readiness` and covered by `npm run test:mission-readiness`. It is a read-only final PRD gate: required package scripts must exist, safety-negative evidence must come from `.rescuegrid/safety-negative-report.json`, wallet click-through evidence must verify against Sui create/revoke events, execution funding readiness must pass through the same `buildExecutionReadiness` contract as `/api/execution/readiness` / `funding:watch`, and strict execution must provide `.rescuegrid/demo-execute-report.json` from `npm run demo:execute:report` with `G2-EXECUTE`, `tick_outcome=executed`, `AgentTradeExecuted` evidence, `execution_claimed=true`, spend increase and a tick tx digest. It intentionally returns blocked in the current repo state until the safety report artifact, DBUSDC/DEEP funding, filled browser-wallet evidence and strict execution evidence exist.
...
Current H/K accounts implementation status: Profile consumes the Worker `/api/runtime/status` read surface and shows the runtime signer kind, deployment agent, execution blocker and chain data-provider mode. This is status transparency only; it does not enable `waap`, hardware or remote signer execution.

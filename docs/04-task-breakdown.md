...
| G7 | Commit | 1h | Browser wallet evidence artifact | `npm run wallet:evidence -- --format markdown --out .rescuegrid/wallet-clickthrough-evidence.md` 生成只读、gitignored 的 Slush/std wallet create/revoke 证据模板，列出 create tx、wrapper、mandate、revoke tx 和截图字段；生成模板不得被当作已完成 click-through |
...
Current G7 implementation status: `scripts/wallet-clickthrough-evidence.mjs` is exposed at root as `npm run wallet:evidence`. It is read-only: it can fetch public Worker root/runtime/readiness/chain-data posture, writes a gitignored `.rescuegrid/wallet-clickthrough-evidence.md` artifact, keeps `actual_clickthrough_completed=false` until manually filled, and records required owner/create/revoke/wrapper/mandate/screenshot evidence fields without claiming DeepBook execution.

...
| G8 | Commit | 1h | Mission readiness gate | `npm run mission:readiness` 汇总 validation scripts、真实钱包证据、funding readiness 和 strict `AgentTradeExecuted` report；只有全部通过才返回 `status=ready`，当前资金/钱包证据未完成时必须返回 `status=blocked` 且不得创建 policy 或提交 PTB |
...
Current G8 implementation status: `scripts/mission-readiness.mjs` is exposed at root as `npm run mission:readiness` and covered by `npm run test:mission-readiness`. It is a read-only final PRD gate: required package scripts must exist, wallet click-through evidence must verify against Sui create/revoke events, execution funding readiness must pass through the same `buildExecutionReadiness` contract as `/api/execution/readiness` / `funding:watch`, and strict execution must provide `.rescuegrid/demo-execute-report.json` from `npm run demo:execute:report` with `G2-EXECUTE`, `tick_outcome=executed`, `AgentTradeExecuted` evidence, `execution_claimed=true`, spend increase and a tick tx digest. It intentionally returns blocked in the current repo state until DBUSDC/DEEP funding, filled browser-wallet evidence and strict execution evidence exist.

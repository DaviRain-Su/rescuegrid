...
| G7 | Commit | 1h | Browser wallet evidence artifact | `npm run wallet:evidence -- --format markdown --out .rescuegrid/wallet-clickthrough-evidence.md` 生成只读、gitignored 的 Slush/std wallet create/revoke 证据模板，列出 frontend preflight、Worker public state、runtime/readiness signer posture、create tx、wrapper、mandate、strategy hash、activation strategy file、strict execution report reference、revoke tx 和截图字段；`npm run wallet:evidence:preflight` 在手工 QA 前要求本地 frontend、登录边界和 Worker public state 就绪；`npm run wallet:evidence:verify -- --input .rescuegrid/wallet-clickthrough-evidence.md --require-worker --execution-report .rescuegrid/demo-execute-report.json` 校验填好的 tx/event 一致性、activation strategy JSON hash/lifecycle、strict report 引用路径和 Worker detail create/revoke activity 同步；生成模板或 preflight 通过不得被当作已完成 click-through |
| G8 | Commit | 1h | Mission readiness gate | `npm run mission:readiness` 汇总 validation scripts、safety-negative report、真实钱包证据、funding readiness、target funding proof report 和 structured strict `AgentTradeExecuted` report；`npm run demo:execute:wallet-report` 必须能对钱包创建且仍 active 的同一个 wrapper 执行 strict tick，等待浏览器钱包撤销后写 `.rescuegrid/demo-execute-report.json`；`npm run mission:readiness:report` 持久化同一份 JSON 到 `.rescuegrid/mission-readiness-report.json`；只有全部通过才返回 `status=ready`，当前资金/钱包/执行证据未完成时必须返回 `status=blocked` 且不得创建 policy 或提交 PTB |

Current G5 implementation status: `worker/scripts/funding-handoff.mjs`, `worker/scripts/funding-watch.mjs`, root `npm run funding:request`, `npm run funding:watch` and `npm run funding:watch:report` reuse `buildExecutionReadiness` to produce a read-only external funding request/watch. The output includes public agent / BalanceManager ids, DeepBook pool, DBUSDC/DEEP coin types, observed/required/missing amounts, signer readiness, a public signer capability matrix, public WaaP external-signer posture, blocker codes and the next `npm run daemon -- status --json` / `npm run funding:watch -- --json` / `npm run funding:watch:report` / `npm run demo:execute` verification commands. Signer capability and external signer rows are allowlisted before serialization so raw permission tokens, sessions, runner output and key material cannot pass through from future readiness implementations. `npm run funding:request -- --format markdown --out .rescuegrid/funding-request.md` writes the same secret-safe handoff as a gitignored provider artifact, and `npm run funding:watch:report` writes `.rescuegrid/funding-watch-report.json` with the latest blocked/ready execution gate. It does not create policies, submit PTBs or print secrets while blocked.
...
Current G7 implementation status: `scripts/wallet-clickthrough-evidence.mjs` is exposed at root as `npm run wallet:evidence`, `npm run wallet:evidence:preflight` and `npm run wallet:evidence:verify`. It is read-only: it can fetch frontend reachability, source-level login guardrails, public Worker root/runtime/readiness/chain-data posture, signer capability kind list, selected signer capability and WaaP external-signer posture, writes a gitignored `.rescuegrid/wallet-clickthrough-evidence.md` artifact, keeps `actual_clickthrough_completed=false` until manually filled, records required owner/create/revoke/wrapper/mandate/screenshot evidence fields plus `activation_strategy_file` and `strict_execution_report_reference`, and verifies a filled artifact only after `actual_clickthrough_completed=true` plus sign-in/create/revoke/active/revoked screenshot or evidence references and the strict report reference are present. The live wallet create path now emits an Activation strategy evidence JSON artifact in the UI after successful `PolicyCreated` + activation; operators record that downloaded JSON path as `activation_strategy_file` for `demo:execute:wallet-report`. The verifier reads that file, rejects secret material, recomputes the canonical strategy hash and compares available owner/wrapper/mandate/create-digest metadata before it reads Sui create/revoke transactions. It also rejects obvious secret assignments in the main artifact, defaults CLI `--verify` to `--require-worker --execution-report .rescuegrid/demo-execute-report.json`, checks the strict report reference against that expected path, then checks Sui `PolicyCreated` / `PolicyRevoked` transaction events plus Worker detail create/revoke activity and revoked-state reads without claiming DeepBook execution. Lower-strength local debugging must pass `--skip-worker-detail` or `--skip-strict-execution-report` explicitly, and the root package script is forbidden from using either skip flag by `mission:readiness`. The final `mission:readiness` gate requires Worker detail evidence and passes the actual strict execution report path into the verifier. The preflight path fails before manual QA if the local frontend, explicit no-auto-connect login boundary or Worker public state is not ready.

Current G8 implementation status: `scripts/mission-readiness.mjs` is exposed at root as `npm run mission:readiness` / `npm run mission:readiness:report` and covered by `npm run test:mission-readiness`. It is a read-only final PRD gate: required package scripts must exist, safety-negative evidence must come from `.rescuegrid/safety-negative-report.json` and prove live Sui Testnet active/expired wrapper ids plus revoke tx, wallet click-through evidence must verify against Sui create/revoke events and Worker detail activity, execution funding readiness must pass through the same `buildExecutionReadiness` contract as `/api/execution/readiness` / `funding:watch` and include signer capability / external-signer posture, `.rescuegrid/funding-proof-report.json` must prove provider tx success plus target BalanceManager / agent gas evidence with `funding_proven=true`, and strict execution must provide `.rescuegrid/demo-execute-report.json`. For final browser-wallet readiness the report should come from `npm run demo:execute:wallet-report`, which validates the exact parsed strategy JSON against the on-chain wallet-created wrapper hash, activates/forces the existing wrapper, waits for browser-wallet revoke at `awaiting_wallet_revoke`, and writes the full create -> execute -> wallet revoke -> post-revoke loop report without creating or revoking the policy itself. That strict report must include Testnet purpose/chain metadata, wrapper/mandate/strategy ids, successful create and revoke txs with timestamps, distinct create/tick/revoke digests, all G2 loop assertions, structured `AgentTradeExecuted` event evidence for the same wrapper/mandate/tick digest, `execution_claimed=true`, spend increase, create <= execute <= revoke timestamp order, and post-revoke `POLICY_REVOKED` / `execution_claimed=false` no-execution evidence with create/execute/revoke event types. The report variant writes the aggregate readiness JSON even when status is `blocked`, so reviewers can inspect the exact remaining blockers without treating the artifact as completion. It intentionally returns blocked in the current repo state until DBUSDC/DEEP funding, target funding proof, filled browser-wallet evidence and strict execution evidence exist; fresh checkouts must also regenerate the gitignored safety report artifact.
...
| J7 | Explore | 4h | WaaP/external signer adapter design | Draft `SignerAdapter` for `worker-secret`, `cloud-per-user`, `local-daemon`, `waap`, `hardware`, `remote-signer`; validate whether `cloud-per-user` can stay status-only until Seal/Walrus key provisioning is proven and whether `waap-cli send-tx --tx-json ... --chain sui:testnet` can sign a Guardian-approved RescueGrid PTB; require security review before production submission |
...
## Phase L — Post-MVP Per-User Cloud Agent Key

Phase L implements the per-user agent key architecture designed in [`docs/02-architecture.md`](02-architecture.md) §6. It depends on J5 (Seal + Walrus validated) and J7 (SignerAdapter boundary). MVP uses a single shared agent key for all users; Phase L eliminates the cross-user blast radius by giving each user their own Sui agent keypair, encrypted via Seal and stored on Walrus.

| ID | Type | Estimate | Task | Acceptance |
| --- | --- | --- | --- | --- |
| L1 | Explore | 4h | Seal encryption + Walrus storage spike | Prove end-to-end: generate ed25519 keypair → Seal-encrypt → store blob on Walrus → read back → decrypt → sign a test PTB. Confirm Worker identity can be the Seal authorized decryptor. |
| L2 | Commit | 3h | Per-user key generation at registration | zkLogin / wallet connect → Worker generates per-user Sui keypair → registers MoveGate AgentPassport on-chain → encrypts private key via Seal → stores on Walrus → persists Seal access policy id + Walrus blob id in user's Durable Object metadata |
| L3 | Commit | 3h | Update Policy creation to use per-user agent | `create_policy` PTB writes user's own agent address into `mandate.agent` instead of the deployment-level shared `RESCUEGRID_AGENT_ADDRESS`. RescuePolicyWrapper also records user's agent. |
| L4 | Commit | 3h | Per-user key signing in tick path | Tick reads Seal/Walrus refs from DO → decrypts user's agent key → signs PTB with per-user key → key material cleared after signature. Falls back to shared key for legacy Policies. |
| L5 | Commit | 2h | SignerAdapter: `cloud-per-user` kind | New signer kind alongside `worker-secret`. `/api/runtime/status` exposes whether the deployment uses shared or per-user signing. |
| L6 | Commit | 2h | Dashboard: user agent identity display | Profile shows user's own agent address + AgentPassport id. Policy Inspect shows per-user agent, not a shared platform address. |
| L7 | Commit | 2h | Migration and fallback tests | Shared-key Policies continue to work. New Policies use per-user keys. Revoking an old Policy and recreating it migrates to per-user key. Tick correctly selects key based on Policy's agent address. |

Current L5 implementation status: the `cloud-per-user` signer kind and public posture are implemented as a disabled boundary only. `/api/runtime/status`, `/api/execution/readiness`, funding handoff/watch/proof, mission readiness, wallet evidence, Profile signer-health and `npm run daemon -- status --json` can all show that per-user cloud signing is planned but not validated. This does not satisfy L1-L4, L6 or L7: no per-user key is generated, no AgentPassport is registered, no Seal/Walrus key blob is written or decrypted, and no new Policy uses a per-user agent address yet.
...
Polish 可延后：D1 的完整视觉打磨、D6 的细粒度空状态、B7 pi-worker 深度验证、B8 LeafSheep/CDPM 深度验证、F4 幂等增强、Phase H、Phase I、Phase J、Phase K、Phase L、Post-MVP browser QA。
...
15. Phase L depends on J5 (Seal + Walrus validated: encrypted write/read, ACL and chain-anchor) and J7 (SignerAdapter boundary with per-key selection). It must not enter MVP critical path.

...
| G7 | Commit | 1h | Browser wallet evidence artifact | `npm run wallet:evidence -- --format markdown --out .rescuegrid/wallet-clickthrough-evidence.md` 生成只读、gitignored 的 Slush/std wallet create/revoke 证据模板，列出 frontend preflight、Worker public state、create tx、wrapper、mandate、revoke tx 和截图字段；`npm run wallet:evidence:preflight` 在手工 QA 前要求本地 frontend、登录边界和 Worker public state 就绪；`npm run wallet:evidence:verify -- --input .rescuegrid/wallet-clickthrough-evidence.md` 校验填好的 tx/event 一致性；生成模板或 preflight 通过不得被当作已完成 click-through |

...
- report 必须包含 post-revoke evidence：`post_revoke.action=stopped_revoked`、`post_revoke.code=POLICY_REVOKED`、`post_revoke.execution_claimed=false`、`post_revoke.final_policy_status=revoked`、`post_revoke.final_runtime_state=Revoked`，且 `post_revoke.chain_event_types` 必须包含 `PolicyCreated`、`AgentTradeExecuted` 和 `PolicyRevoked`。这些事件类型必须能从 Worker detail 的原始 `events`、`chain_activity` 或聚合 `activity` feed 中提取；ChainDataProvider 输出形态变化不得导致已存在的链上事件在最终 strict report 中丢失。
...
- 即使当前 live readiness 已经通过，successful tx digest 若只命中 agent gas、只命中部分 BalanceManager asset，或只通过非 deposit MoveCall 带出 DBUSDC/DEEP type argument，不足以证明 DBUSDC/DEEP provider funding；report 必须 `status=failed`、`funding_proven=false`，并暴露 `FUNDING_TX_REQUIRED_TARGET_ASSET_NOT_PROVEN`、`transaction_evidence.required_target_asset_evidence_passed=false` 和 `missing_required_target_assets`。
...
- 支持 `--tx` / `--digest` / `--dbusdc-tx` / `--deep-tx` / `--sui-gas-tx`，允许 provider 使用一笔或多笔交易完成交付。Role-specific digest 必须证明对应目标资产：`--dbusdc-tx` 需要 DBUSDC target hit，`--deep-tx` 需要 DEEP target hit，`--sui-gas-tx` 需要 agent gas SUI_MIST target hit。角色和资产错配时 report 必须 `status=failed`、`funding_proven=false`，并暴露 `FUNDING_TX_ROLE_ASSET_NOT_PROVEN`、`transaction_evidence.role_asset_evidence_passed=false` 和 `failed_role_asset_digests`。

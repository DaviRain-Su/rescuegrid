...
- `npm run wallet:evidence:preflight` must fail before manual wallet QA if the local frontend is unreachable, wallet auto-connect is not disabled, the Worker read-only entry is not explicit, no-wallet demo entry is not explicit, Worker writes do not require Worker config, public Worker status/readiness endpoints are unavailable, or those Worker endpoints do not semantically match the expected RescueGrid Testnet service, deployed agent, Worker-first chain-data posture, preflight-only execution readiness and non-secret external-signer posture. Passing preflight is only environment evidence, not click-through proof.

...
- Browser wallet evidence must include a filled `.rescuegrid/wallet-clickthrough-evidence.md` artifact from `npm run wallet:evidence` plus a passing `npm run wallet:evidence:verify -- --input .rescuegrid/wallet-clickthrough-evidence.md --require-worker`; `npm run wallet:evidence:preflight` should pass before the manual browser run, and the same wrapper must stay active for strict execution evidence before the wallet revoke step. The generated blank artifact or preflight pass alone is not a passing click-through proof.

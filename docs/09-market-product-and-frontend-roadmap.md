...
Implementation status: `worker/src/sui-protocol-registry.js` and `/api/protocols` expose the current Sui-only coverage baseline refreshed on 2026-06-02 UTC from DefiLlama `protocols` plus the Sui DEX volume overview. The registry uses `chainTvls.Sui` for top-26 ranking, includes Turbos as a volume exception, and marks only DeepBook V3 as the configured executor. `worker/src/sui-watch-registry.js` and `/api/protocols/watchlist` map that protocol universe into 31 Sui-only market rows with protocol, venue, market, risk, adapter and data-source metadata. DeepBook rows are `live_executor_funding_gated`, with `executor_configured=true`, `execution_enabled=false` and `execution_blocker_code=FUNDING_GATED` until DBUSDC/DEEP funding is real. `worker/src/sui-adapter-candidates.js` and `/api/adapters/candidates` now define H4/H5 adapter constraints for Cetus, Turbos, Momentum, Bluefin Spot, NAVI, Suilend, Scallop and AlphaLend. These rows are not executor registrations; they specify target schemas, read fields, preflight gates and wrapper changes while keeping `registered_executor=false` and `execution_enabled=false`. `worker/src/sui-watch-only-boundaries.js` and `/api/protocols/watch-boundaries` define H6 watch-only boundaries for Bucket, Current, SpringSui, Haedal, Volo, AlphaFi, Kai, Mole, Ondo, KAIO, MatrixDock, Ember, Bluefin Pro, Sudo and DipCoin, including readable state, risk domains, future target fields and no-execution reasons. DipCoin remains `roadmap_only` until it enters the current Sui coverage baseline or an official read API is verified. H7 is implemented as an adapter SDK skeleton in `worker/src/executor-adapter-sdk.js`; `worker/src/deepbook-adapter.js` is the first plugin and `worker/src/executor-adapters.js` is the registry assembly point. H8 is implemented at `worker/src/sui-dex-read-adapters.js` and `/api/adapters/dex-reads`: DeepBook, Cetus, Turbos, Momentum and Bluefin Spot expose quote/depth/spread read schemas plus a 10-row SUI/USD-stable spread matrix without enabling non-DeepBook execution. H9 is implemented at `worker/src/sui-lending-read-adapters.js` and `/api/adapters/lending-reads`: NAVI, Suilend, Scallop and AlphaLend expose reserve/obligation/health-factor read schemas plus a 4-row borrow-health matrix without enabling repay/withdraw/borrow execution. The frontend now consumes H8/H9 through `src/queries/adapter-surfaces.js`; Data Sources shows both Worker adapter surfaces, Markets shows the DEX read boundary on Sui DEX Spreads and the lending health boundary on Yield Monitor, and Strategy Marketplace/Detail shows template-level adapter blockers and read-only permission boundaries.
...
Implementation status: `src/components/Risk.jsx` now implements the K7 Risk Center surface. It shows global budget and daily loss caps, per-strategy pause/resume controls, Sui venue stop/resume controls, per-venue exposure caps, liquidation watch, oracle health, signer/executor health, stale-data warning summaries, Guardian preset editing/simulation and the capability matrix. In live wallet mode, global / strategy / venue stop/resume signs a Sui personal message and writes `/api/risk/controls`; Worker ticks read the persisted owner-scoped controls and block matching submissions with `GLOBAL_STOPPED`, `STRATEGY_STOPPED` or `VENUE_STOPPED` before PTB signing. Demo mode keeps a local preview state.
...
Implementation status: Profile now consumes `/api/runtime/status` when live Worker reads are enabled. The surface shows the deployment agent address, signer kind, execution blocker and chain data-provider mode next to policy/funding readiness. This covers the minimum cloud-agent transparency needed for the hackathon branch. J7 now has a local-daemon-only WaaP CLI signer boundary, but the UI still needs a richer follow-up state for daemon liveness, WaaP approval status, privilege scope and external signer health.
...
- Current signer status transparency is implemented in Profile via `/api/runtime/status`; the local-daemon WaaP CLI boundary is now test-covered, while live WaaP approval/privilege UX and production execution remain future work.
